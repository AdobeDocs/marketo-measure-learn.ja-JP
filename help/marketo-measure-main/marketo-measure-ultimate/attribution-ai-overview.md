---
title: Attribution AI の概要
description: Adobeのアトリビューション AIが、高度なAI/マシンラーニングモデルによりB2B マーケティングの測定をどのように強化するかをご確認ください。
role: User
level: Beginner
doc-type: Feature Video
duration: 0
last-substantial-update: 2024-08-22T00:00:00Z
jira: KT-15733
feature: Attribution
exl-id: 31e20359-bc42-444f-962a-bf33ad417fd7
source-git-commit: 557fe2c83a26425c5c849e6260a416215ce1c7fb
workflow-type: tm+mt
source-wordcount: '44'
ht-degree: 68%

---

# Attribution AI の概要

アドビのアトリビューション AI が、高度な AI/ML モデリングを使用して B2B マーケティング測定を強化し、正確なアトリビューションに関するインサイトを提供し、正確な分析のためにバイアスを排除する方法を説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3433052/?learn=on)
